	...
	class YoungDuke : ModNPC
	{
        ...
        public override void AI()
        {
			...
			else if (npc.ai[0] == 3f)
			{
				...
				if (Main.netMode != 1 && npc.ai[2] == (float)(num36 - 30))
				{
					Vector2 vector4 = npc.rotation.ToRotationVector2() * (Vector2.UnitX * npc.direction) * (npc.width + 20) / 2f + center;
					Projectile.NewProjectile(vector4.X, vector4.Y, npc.direction * 2, 8f, 385, 0, 0f, Main.myPlayer);
					Projectile.NewProjectile(vector4.X, vector4.Y, -npc.direction * 2, 8f, 385, 0, 0f, Main.myPlayer);
				}
				npc.ai[2] += 1f;
			...
			else if (npc.ai[0] == 7f)
			{
				...
				if (npc.ai[2] % (float)num6 == 0f)
				{
					...
					if (Main.netMode != 1)
					{
						...
						int num25 = NewNPC((int)vector6.X, (int)vector6.Y + 45, 371);
...

			{
				...
				if (Main.netMode != 1 && npc.ai[2] == (float)(num36 - 30))
				{
					Projectile.NewProjectile(center.X, center.Y, 0f, 0f, 385, 0, 0f, Main.myPlayer, 1f, npc.target + 1);
				}
				npc.ai[2] += 1f;
				if (npc.ai[2] >= (float)num36)
				{
			...
			else if (ai[0] == 12f)
			{
				...
				if (Main.netMode != 1 && npc.ai[2] == (float)(num4 / 2))
				{
					if (npc.ai[1] == 0f)
					{
						npc.ai[1] = 300 * Math.Sign((center - player.Center).X);
