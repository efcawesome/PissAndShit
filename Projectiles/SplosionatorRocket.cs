        ...
        public override void AI()
        {
            ...
            if (projectile.ai[1] == 0f)
            {
                for (int num171 = 0; num171 < 200; num171++)
                {
                    ...
                    if (!(num174 < num169) || !Collision.CanHit(new Vector2(projectile.position.X + projectile.width / 2f, projectile.position.Y + projectile.height / 2f), 1, 1, Main.npc[num171].position, Main.npc[num171].width, Main.npc[num171].height))
                    {
                        continue;
                    }
