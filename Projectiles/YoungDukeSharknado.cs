        ...
        {
			{
				...
				if (projectile.localAI[0] == 0f && Main.myPlayer == projectile.owner)
				{
					...
					projectile.scale = ((float)(num771 + num772) - projectile.ai[1]) * num773 / (float)(num772 + num771);
					projectile.width = (int)((float)num775 * projectile.scale);
					projectile.height = (int)((float)num776 * projectile.scale);
					projectile.position.X -= projectile.width / 2;
					projectile.position.Y -= projectile.height / 2;
				...
				if (projectile.ai[0] == 1f && projectile.ai[1] > 0f && projectile.owner == Main.myPlayer)
				{
					...
					center.Y -= (float)num776 * projectile.scale / 2f;
					float num777 = ((float)(num771 + num772) - projectile.ai[1] + 1f) * num773 / (float)(num772 + num771);
					center.Y -= (float)num776 * num777 / 2f;
					center.Y += 2f;
				...
				if (projectile.ai[0] <= 0f)
				{
					...
					float num782 = (float)projectile.width / 5f;
					if (projectile.type == 386)
					{
						num782 *= 2f;
					}
					float num783 = (float)(Math.Cos(num781 * (0f - projectile.ai[0])) - 0.5) * num782;
					projectile.position.X -= num783 * (float)(-projectile.direction);
					projectile.ai[0]--;
